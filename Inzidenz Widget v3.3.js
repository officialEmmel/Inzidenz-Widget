...
function getTrend(inz)
{
  ...
  else 
  {
     ...
     return {"icon":ic, "text":"Covid-19 Inzidenz", "color":fontColor}
...
//      }
  ...
  if(dif == 0) {s = SFSymbol.named(ICON.arrowHorizontal())}
...
function diagram(w)
{
  ...
  let defaultWert = Math.min(...data) + 0.123
  let versatz = 0
  ...
  for(let i = 0; i < dates.length; i++) {
    ...
    let dif = date - lastDate
    dif = (dif/86400000).toFixed(0)
//     log(date.toLocaleDateString() + ": " + dif)  
    for(let i2 = 1; i2 < (dif); i2++){
      data.insert(i + versatz, defaultWert)
      versatz += 1
      completeData = false
    }
    lastDate=date   
  ...
  for(let i = 0; i < max; i++)
  {
    ...
    path.addRoundedRect(new Rect(pos, canvasSize.height, blockW, -height), blockW/2, blockW/2)
    ...
    else if(n < 750)
    {
      ...
      canvas.setFillColor(new Color("#0c3a91"))
    }
    else
    {
      canvas.setFillColor(new Color("#631b03"))
    }
    canvas.fillPath()
